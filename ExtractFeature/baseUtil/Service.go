...
const (
	INIT = iota + 1 // Nothing happened yet.
	...
	// TCP specific
	S0 // +Connection attempt seen, no reply. 尝试连接，没有应答
	S1 // +Connection established, not terminated. 连接建立，但没有终止
	S2 // +Connection established and close attempt by originator seen (but no reply from responder).
	//连接建立，并看到发起者尝试关闭（但未收到响应者回复）
	S3 // +Connection established and close attempt by responder seen (but no reply from originator).
	//连接建立，响应者尝试关闭（但是并未收到发送者回复）
	REJ    // +Connection attempt rejected. 尝试连接被拒绝
